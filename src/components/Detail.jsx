...
const Detail = () => {
  ...
  useEffect(() => {
    async function getData() {
      ...
      setData(_data.data());
      setForm(_data.data()); // Set initial form state with current movie data
      setLoading(false);
    }
    getData();
  }, [id]);

  // Open modal to edit movie details
  const openModal = () => {
    setModalOpen(true);
  };
  ...
  // Handle update button click to update movie data
  const updateMovie = async () => {
    try {
      const movieRef = doc(db, "movies", id);
      await updateDoc(movieRef, form); // Update Firestore document
      closeModal(); // Close modal
      alert("Movie updated successfully!"); // Optionally show a success message
    } catch (error) {
  ...
  return (
    <div className='p-4 mt-4 flex flex-col md:flex-row items-center md:items-start w-full justify-center'>
      ...
      {/* Modal to edit the movie details */}
      {modalOpen && (
